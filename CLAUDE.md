...
- 🖼️ **이미지 역할 분담(9/20 사장님)**: **AI 이미지엔 글자 0**(사장님이 나노바나나로 생성 · 프롬프트 `doc/이미지_프롬프트_채크소개.md` · 폰 화면은 꺼진 검은 유리) → **글자 조판은 클로드 디자인에서 사장님**(`doc/디자인_프롬프트_채크소개.md`) → **저는 이미지를 포토샵처럼 손본다**(`tools/img.py` · 합성 경로 9/20 검증됨). 이미지가 오면 그게 정본(마음푸리 D-019)
